        ...
        while change :
            ...
            for j in np.arange(n):
                for h in np.arange(s):
                    CAF[j][h] = (A_alpha_kk__A_alpha)[h]*(T_1).toarray()[h][j] - (F_alpha_kk__F_alpha)[h]*(T_2.toarray())[h][j] + A_alpha*(T_3)[h][j]
            ...
            c =  sum_k + sum_h + np.sum(tr) + A_alpha*SS.sum() - F_alpha*SS_n.sum()
