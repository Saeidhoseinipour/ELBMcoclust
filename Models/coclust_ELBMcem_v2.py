...
class CoclustELBMcem:
    ...
    def _fit_single(self, X, random_state, y=None) :
        ...
        D_A_F = np.nan_to_num(SS/SS_n , posinf = 0)
        ...
        if (self.model == "Poisson"):
            ...
            A_alpha = np.nan_to_num((D_A_F)/(1-D_A_F), posinf = 0)
            A_alpha[A_alpha<0] = 0
            A_alpha = sp.csr_matrix(A_alpha)
            A_alpha.data = np.log(A_alpha.data)

            F_alpha = np.nan_to_num(1-D_A_F, posinf = 0)
            F_alpha[F_alpha<0] = 0
            F_alpha = sp.csr_matrix(F_alpha)
            F_alpha.data = -np.log(F_alpha.data)
        elif (self.model == "Beta"):
            A_alpha = sp.csr_matrix(D_A_F)
            F_alpha = sp.csr_matrix(D_A_F)
        ...
        while change :
            ...
            D_A_F = np.nan_to_num(SS/SS_n+const , posinf = 0)
            #print("alpha_hat"+str(D_A_F))
            if (self.model == "Poisson"):
                D_A_F[D_A_F<0] = 0
                D_A_F[D_A_F>1] = 0
            ...
            #print(type(SS),type(SS_n),type(A_alpha),type(F_alpha))   
            #print(A_alpha*SS)
            #print(F_alpha*SS_n)
            #minval=np.min(A_alpha[np.nonzero(A_alpha)]) 
            #A_alpha[A_alpha == 0] = minval*0.00000001
            ...
            Ad = (A_alpha.toarray()*SS.toarray()).sum() - (SS_n.toarray()*F_alpha.toarray()).sum()
            #print("Ad"+str(Ad))
            #T1 = SS.T@A_alpha
            #print(np.diag(SS_n.T@F_alpha))
            #tr = np.trace(T1) # - np.trace(T2)
            sum_k = R.sum(axis=0)@np.log(pi_k).T 
            sum_h = C.sum(axis=0)@np.log(rho_h).T
