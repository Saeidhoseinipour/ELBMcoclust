...
class CoclustPLBcem:
    ...
    def _fit_single(self, X, random_state, y=None) :
        ...
        while change :
            ...
            Z_cluster = sp.lil_matrix((Z1.shape[0],K))
            Z_cluster[np.arange(Z1.shape[0]), np.sort(Z1.argmax(axis = 1).A1)] = 1
            ...
            W_cluster = sp.lil_matrix((W1.shape[0],L))
            W_cluster[np.arange(W1.shape[0]), np.sort(W1.argmax(axis = 1).A1)] = 1
            ...
            minval=np.min(gammakl[np.nonzero(gammakl)]) 
            gammakl[gammakl == 0] = minval*0.00000001
            print("alpha_kh==========>"+str(gammakl.toarray()))
        ...
        self.row_labels_ = [x+1 for x in Z.toarray().argmax(axis=1).tolist()] 
        self.column_labels_ = [x+1 for x in W.toarray().argmax(axis=1).tolist()]
        self.rowcluster_matrix = Z_cluster.toarray()@Z_cluster.toarray().T@X.toarray()
        self.columncluster_matrix = X.toarray()@W_cluster.toarray()@W_cluster.toarray().T
        self.reorganized_matrix = Z_cluster.toarray()@Z_cluster.toarray().T@X.toarray()@W_cluster.toarray()@W_cluster.toarray().T        
