...
class CoclustSELBMcem:
    ...
    def _fit_single(self, X, random_state, y=None) :
        ...
        D_A_F_alpha_kk = np.nan_to_num(SS.toarray()/(SS_n.toarray()+const))          #+ self.tol
        D_A_F_alpha_kk = np.diag(D_A_F_alpha_kk)
        D_A_F_alpha = np.nan_to_num(SS_a/(SS_a_n+const))          #+ self.tol
        ...
        while change :
            ...
            for i in np.arange(m):
                for k in np.arange(g):
                    RAF[i][k] = (A_alpha_kk__A_alpha)[k]*(T_1.toarray())[i][k]
            ...
            if (self.model == "Bernoulli"):
                ...
                A_alpha_kk__A_alpha = np.nan_to_num((D_A_F_alpha_kk)/(D_A_F_alpha))    # Domin alpha?5
                #A_alpha_kk__A_alpha[A_alpha_kk__A_alpha<0] = 0
                #A_alpha_kk__A_alpha = sp.csr_matrix(A_alpha_kk__A_alpha)
                A_alpha_kk__A_alpha = np.log(A_alpha_kk__A_alpha)
            ...
            for j in np.arange(n):
                for h in np.arange(s):
                    CAF[j][h] = (A_alpha_kk__A_alpha)[h]*(T_1.toarray())[h][j]  
        ...
        self.reorganized_matrix_2 = np.diag(np.max(R_cluster.toarray()@R_cluster.toarray().T@X.toarray()@C_cluster.toarray()@C_cluster.toarray().T , axis = 1))                     
        #self.A_star = A_star
        #self.F_star = F_star
        self.R = R_cluster
        self.C = C_cluster