...
class CoclustSELBMcem:
    ...
    def _fit_single(self, X, random_state, y=None) :
        ...
        if (self.model == "Bernoulli"):
            A_alpha_kk__A_alpha = np.nan_to_num(((1- D_A_F_alpha)*(D_A_F_alpha_kk))/((1-D_A_F_alpha_kk)*(D_A_F_alpha)), posinf = 0)  #+ self.tol
            A_alpha_kk__A_alpha[A_alpha_kk__A_alpha<=0] = 1
            A_alpha_kk__A_alpha = np.nan_to_num(np.log(A_alpha_kk__A_alpha), posinf = 0)                # values between 0 to 1 is negative 
            ...
            A_alpha_kk__A_alpha = A_alpha_kk__A_alpha.toarray()
        ...
        while change :
            ...
            for i in np.arange(m):
                for k in np.arange(np.min((g,s))):
                    ...
                    RAF[i][k] = (A_alpha_kk__A_alpha)[k][k]*(T_1.toarray())[i][k] - (F_alpha_kk__F_alpha)[k][k]*(T_2.toarray())[i][k] + A_alpha*(T_3)[i][k]
            ...
            for j in np.arange(n):
                for h in np.arange(np.max((g,s))):
                    if h < np.min((g,s)):     
                        ...
                        #print(A_alpha*T_3)
                        CAF[j][h] = (A_alpha_kk__A_alpha.T@T_1.toarray())[h][j] - (F_alpha_kk__F_alpha.T@T_2.toarray())[h][j] 
                    else:
        ...
        self.reorganized_matrix_3 = R_cluster.toarray()@np.diag(np.max(R_cluster.toarray().T@X.toarray()@C_cluster.toarray(), axis = 0))@C_cluster.toarray().T                        
        #self.A_star = A_star
        #self.F_star = F_star
        self.R = R_cluster
        self.C = C_cluster