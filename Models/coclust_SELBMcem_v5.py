...
class CoclustSELBMcem:
    ...
    def _fit_single(self, X, random_state, y=None) :
        ...
        D_A_F_alpha_kk = np.nan_to_num(SS/SS_n )          #+ self.tol
        ...
        while change :
            ...
            ### CE step
            A_star =  np.diag(np.max(A_alpha_kk__A_alpha.toarray(), axis = 1)) + (A_alpha*(E_gg-I_gg))
            #print(A_star)
            One = np.ones((g,s-g))
            One_A = A_alpha*One
            A_star = np.hstack((A_star,One_A))   
            A_star = sp.lil_matrix(A_star)

            RA = A_star@(S.multiply(beta)@C).T

            F_star  =  np.diag(np.max(F_alpha_kk__F_alpha.toarray(), axis = 1)) + (F_alpha*(E_gg-I_gg))
            One_F = F_alpha*One
            F_star = np.hstack((F_star,One_F))
            ...
            D_A_F_alpha_kk = np.nan_to_num(SS/SS_n)
            ...
            A_star =  np.diag(np.max(A_alpha_kk__A_alpha.toarray(), axis = 1)) + (A_alpha*(E_gg-I_gg))         #(k,k)
