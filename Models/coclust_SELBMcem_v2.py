...
class CoclustSELBMcem:
    ...
    def _fit_single(self, X, random_state, y=None) :
        ...
        #print(SS,SS_n)
        #print(SS,SS_n)
        SS_a = SS.sum()-np.max(SS, axis = 1).sum()
        SS_a_n = SS_n.sum()-np.max(SS_n, axis = 1).sum()

        D_A_F_alpha_kk = np.nan_to_num(SS/SS_n+const)
        ...
        if (self.model == "Bernoulli"):
            ...
            A_alpha = np.log(D_A_F_alpha/(1-D_A_F_alpha))
        ...
        while change :
            ...
            D_A_F_alpha_kk = np.nan_to_num(SS/SS_n+const)
            D_A_F_alpha = np.nan_to_num(SS_a/SS_a_n+const)         # number
