...
class CoclustSELBMcem:
    ...
    def _fit_single(self, X, random_state, y=None) :
        ...
        D_A_F_alpha_kk = np.nan_to_num(SS/SS_n , posinf = self.tol)          #+ self.tol
        D_A_F_alpha = np.nan_to_num(SS_a/SS_a_n , posinf = self.tol)          #+ self.tol

        if (self.model == "Bernoulli"):
            A_alpha_kk__A_alpha = np.nan_to_num(((1- D_A_F_alpha)*(D_A_F_alpha_kk))/((1-D_A_F_alpha_kk)*(D_A_F_alpha)), posinf = self.tol)  #+ self.tol
            A_alpha_kk__A_alpha[A_alpha_kk__A_alpha<0] = 0
            A_alpha_kk__A_alpha = sp.csr_matrix(A_alpha_kk__A_alpha)
            A_alpha_kk__A_alpha.data = np.log(A_alpha_kk__A_alpha.data)                 # values between 0 to 1 is negative 

            A_alpha = np.nan_to_num(np.log(D_A_F_alpha/(1-D_A_F_alpha)), posinf = 0)               # + self.tol
            ...
            A_alpha = np.nan_to_num(np.log(D_A_F_alpha), posinf = 0)
        ...
        while change :
            ...
            ### CE step
            A_star =  np.diag(np.max(A_alpha_kk__A_alpha.toarray(), axis = 1)) + (A_alpha*E_gg)
            #print(A_star)
            Zero = np.zeros((g,s-g))
            ...
            F_star  =  np.diag(np.max(F_alpha_kk__F_alpha.toarray(), axis = 1)) + (F_alpha*E_gg)
            F_star = np.hstack((F_star,Zero))
            ...
            D_A_F_alpha_kk = np.nan_to_num(SS/SS_n , posinf = 0)
            D_A_F_alpha = np.nan_to_num(SS_a/SS_a_n , posinf = 0)         # number
            ...
            A_star =  np.diag(np.max(A_alpha_kk__A_alpha.toarray(), axis = 1)) + (A_alpha*E_gg)         #(k,k)
            ...
            Den_a  = 1/(SS_a_n + self.tol)                        ####v3

            #D_A_F_alpha_kk = np.nan_to_num(SS/SS_n , posinf = 0)
            D_A_F_alpha_kk = SS.multiply(Den)
            #print(D_A_F_alpha_kk)
            D_A_F_alpha = SS_a.multiply(Den_a)
            #print(D_A_F_alpha)
            ...
            tr = np.trace((SS.T@A_star)) - np.trace((SS_n.T@F_star))
        ...
        self.reorganized_matrix = R_cluster.toarray()@A_star@C_cluster.toarray().T   
        self.reorganized_matrix_2 = R_cluster.toarray()@np.hstack((np.diag(np.max(R_cluster.toarray().T@X.toarray()@C_cluster.toarray(), axis = 1)),Zero_g))@C_cluster.toarray().T                     
        self.A_star = A_star
        self.F_star = F_star
        self.R = R_cluster
        self.C = C_cluster