...
class All_EV(object):
	...
	def IntraCen(self, object_model):  
		...
		for h in range(s):
			for h_ in range(h+1, s):
				...
				pass
			pass
		return 0.5*(np.sum(ICAS_r) + np.sum(ICAS_c))
	...
	def Process_EV(self, true_labels, model):
		...
		for i in np.arange(10):
			...
			runtime_ELBM.append(np.array(ELBM.runtime).flatten())
			runtime_SELBM.append(np.array(SELBM.runtime).flatten())

		return   [Acc_row_ELBM, Acc_row_SELBM, ARI_row_ELBM, ARI_row_SELBM, ICAS_ELBM, ICAS_SELBM, runtime_ELBM, runtime_SELBM]